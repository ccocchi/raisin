  ...
  module DSL
    module Api
      %w(get head post put delete).each do |via|
        class_eval <<-EOF, __FILE__, __LINE__ + 1
          def #{via}(path = '/', options = {}, &block)
            ...
            klass = create_action_class(method_name, &block)
      ...
      def single_resource
        ...
        @_prefix = @_prefix.singularize if @_prefix
      end
      ...
      def namespace(path, &block)
        path = path.sub(%r(\A/?#{@_prefix}), '') if @_prefix
      ...
      def eval_included(klass)
        klass.class_eval(&@_included) if @_included
      end

      def create_action_class(method_name, &block)
        self.const_set method_name.camelize, Class.new(Configuration.base_endpoint.constantize, &block)
      end
